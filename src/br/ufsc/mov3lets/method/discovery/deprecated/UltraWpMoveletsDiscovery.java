 ...
 */
public class UltraWpMoveletsDiscovery<MO> extends UltraMoveletsDiscovery<MO> implements TrajectoryDiscovery {
	 ...
	 */
	public UltraWpMoveletsDiscovery(MAT<MO> trajectory, List<MAT<MO>> trajsFromClass, List<MAT<MO>> data, List<MAT<MO>> train, List<MAT<MO>> test,
...
//		candidatesOfSize = candidatesOfSize.stream()
//								  .filter(c -> !candidates.contains(c))
//								  .collect(Collectors.toList());
		candidates.addAll(candidatesOfSize); // Only difference is that we add the movelets of size one to (may repeat) 

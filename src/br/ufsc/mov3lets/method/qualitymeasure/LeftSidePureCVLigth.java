...
//	}
	 ...
	 */
	public Pair<Integer, double[]> getBestSplitpoints(double[][] distances, MO target, List<MO> labels,
	...
	public List<double[]> prunePoints(double[][] distances, MO target, List<MO> labels) {
		 ...
		 * 
		 * @Override public int compare(double[] o1, double[] o2) { for (int i = 0; i <
		 * o1.length; i++) { if (o1[i] != o2[i]) return (o1[i] > o2[i]) ? -1 : 1; }
		 * return 0; } });
		 */
		/*
		 * Set<Integer> indexes = new HashSet<Integer>(); for (int i = 0; i <
		 * candidates.size(); i++) { for (int j = 0; j < rm.getRowDimension(); j++) { if
		 * (Arrays.equals(candidates.get(i), rm.getRow(j))); indexes.add(j); } }
		 * System.out.println(StringUtils.join(indexes,", ")); System.out.println();
		 */
	...
	public Map<String, double[]> getBestSplitpointsCV(Subtrajectory candidate, double[][] distances, MO target,
		...
		for (int i = 0; i < this.samples; i++) {
			 ...
			 */
			Pair<Integer, double[]> bestSplitpoints = getBestSplitpoints(chosePoints.getFirst(), target,
					chosePoints.getSecond(), stairCandidates);
...
//		System.out.println(ct);System.exit(1);
	...
	public double getInformationGain(double[][] distances, List<MO> labels, Map<String, double[]> splitpointsData) {
		...
		/*
		 * First we are going to discretize the distances into a binary vector, where
		 * each value can assume two possible values 1. below the splitpoint 2. above or
		 * equal the splitpoint
		 */
		...
		/*
		 * Secondly we use the Information Gain algorithms of weka to get the
		 * information gain
		 */
...
//					tn++;
	 ...
	 */
	public double getInformationGainMedium(double[][] distances, List<MO> labels,
			Map<String, double[]> splitpointsData) {

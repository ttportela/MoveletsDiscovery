 ...
 */
public class TSInternLoader<T extends MAT<?>> implements InterningLoaderAdapter<T>  {
...
//        System.out.println(classLabels.toString());
    ...
    protected MAT<?> readMultivariateInstance() throws IOException {
		...
		int i = 0, attrCt = 0, composite = -1;
		String value = null;

//		print();
		AttributeDescriptor attr = getAttributeDesc(attrCt);
		composite = compositeSize(attr);
		
		do{
			
			if(m_Tokenizer.ttype == SEQ_SEP && isClass()){
                //this means we're about to get the class value or the next dimension
//				System.out.println((tid-1) + " = " + attrCt + " = " + i);
            	i = 0;
...
//	}
    ...
    protected int compositeSize(AttributeDescriptor attr) {
...
class TempAspect extends Aspect<String> {

	public TempAspect(String value) {
		super(value);
	}
